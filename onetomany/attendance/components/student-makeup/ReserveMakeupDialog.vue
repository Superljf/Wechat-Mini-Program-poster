<template>
  <el-dialog
    ...
    append-to-body
    custom-class="reserve-makeup-dialog"
  >
    <div class="dialog-content">
      ...
      <div class="makeup-info">
        ...
        <el-form :model="form" ref="form" label-width="110px" class="dialog-form">
          ...
          <el-form-item label="预计补课时间" prop="makeupStartTime" :rules="[{ required: true, message: '请选择补课时间', trigger: 'change' }]">
          ...
          <el-form-item label="约课备注" prop="remark">
            <el-input v-model="form.remark" type="textarea" :rows="3" placeholder="请输入备注信息" maxlength="300" show-word-limit></el-input>
          </el-form-item>
        </el-form>
      </div>
    </div>
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="handleConfirm" size="small">确定预约补课</el-button>
    </div>
  </el-dialog>
</template>
  ...
  methods: {
    ...
    handleStartTimeFocus() {
      ...
      if (!this.form.makeupStartTime) {
        ...
        this.form.makeupStartTime = `${currentHour.toString().padStart(2, '0')}:00`
        // 设置开始时间后，自动计算结束时间
        this.calculateEndTime()
      }
    },
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (!this.form.makeupEndTime) {
            this.$message.warning('补课时间计算中，请稍候或重新选择开始时间')
            return
          }

          // 组合日期和时间
          const makeupDateTime = `${this.form.makeupDate} ${this.form.makeupStartTime}-${this.form.makeupEndTime}`
          const submitData = {
            ...this.form,
            makeupTime: makeupDateTime
          }

          // 通过回调函数控制弹窗关闭，只有成功时才关闭
...
// 弹窗样式
::v-deep .el-dialog.reserve-makeup-dialog {
  border-radius: 8px;
  margin-top: 15vh !important;
}

::v-deep .el-dialog.reserve-makeup-dialog .el-dialog__header {
...
::v-deep .el-dialog.reserve-makeup-dialog .el-dialog__header .el-dialog__title {
  font-size: 16px;
  font-weight: 500;
  color: #333;
}

::v-deep .el-dialog.reserve-makeup-dialog .el-dialog__body {
...
::v-deep .el-dialog.reserve-makeup-dialog .el-dialog__footer {
  display: flex;
